...
class database
{
  ...
  public function buscaCliente($telefono, $servicio)
  {
    ...
    if ($servicio == 2) {
      $sql = $this->db->query("SELECT cc.fecha_creacion_orden as mora, (CASE WHEN s.id = '3' THEN 'Pendiente por cobrar' WHEN s.id = 4 THEN 'Pendiente por cobrar' WHEN s.id = 8 THEN 'Pendiente por cobrar' WHEN s.id = 9 THEN 'Compromiso de pago' ELSE s.name END) AS status, cc.status_id, cc.id, cc.id_cuota, cc.local_origen, cc.plata_por_cobrar, cc.fecha_pagar, cc.numero_cuota, cc.tramo_inicial, cc.segmento, cc.nombre_usuario, cc.email, cc.telefono, cc.cedula, (SELECT SUM(CAST(REPLACE(REPLACE(plata_por_cobrar, '.', ''), ',', '.') AS DECIMAL(20, 10))) AS total_plata_por_cobrar FROM cashea_customers WHERE (cedula = '$telefono' OR telefono = '$telefono') AND status_id IN (3,4,8,9,10,12)) AS deuda_total FROM cashea_customers cc  INNER JOIN status s ON cc.status_id = s.id WHERE (cc.cedula = '$telefono' OR cc.telefono = '$telefono') AND cc.status_id IN (3,4,7,8,9,10,12) AND cc.is_active = true ORDER BY cc.id_cuota ASC");
